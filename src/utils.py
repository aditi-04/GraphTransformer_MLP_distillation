...
# from torchmetrics.classification import BinaryPrecisionRecall
...
def precision_recall_batch(output, labels):
    preds = output.max(1)[1].type_as(labels)
    # print (f'preds: {preds}')
    # print (f'labels: {labels}')
    tp = torch.sum((labels == 1) * (preds == 1), dim=0)
    tn = torch.sum((labels == 0) * (preds == 0), dim=0)
    fp = torch.sum((labels == 0) * (preds == 1), dim=0)
    fn = torch.sum((labels == 1) * (preds == 0), dim=0)

    
    precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
    recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    precision = torch.where(tp + fp > 0, precision, torch.tensor(0.0, dtype=torch.float))  # Handle division by zero

    recall = tp / (tp + fn)
    recall = torch.where(tp + fn > 0, recall, torch.tensor(0.0, dtype=torch.float))  # Handle division by zero
...
def re_features_even(adj, features, K):
    #传播之后的特征矩阵,size= (N, 1, K+1, d )
    nodes_features = torch.empty(features.shape[0], 1, K//2+1, features.shape[1])
    ...
    for i in range(0,K,2):
        ...
        for index in range(features.shape[0]):
            ...
            nodes_features[index, 0, i // 2+ 1, :] = x[index]        
